...
namespace MachineControlSystem
{
    public class TubeController
    {
        ...
        public void DispenseProduct(string tubeId, int grams)
        {
            var tube = _tubes.Find(t => t.Id == tubeId);
            if (tube != null)
            {
                if (_sensorController.IsPlatePositionedCorrectly(int.Parse(tube.SensorId)))
                {
